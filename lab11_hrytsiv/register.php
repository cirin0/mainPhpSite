...
if (isset($_SESSION['login'])) {
   $_SESSION['success_message'] = "Щоб зареєструватися, вийдіть з аккаунту";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   if (!isValidName($name) || !isValidName($surname)) {
      ...
      if ($result->num_rows > 0) {
         echo "<p class='error_registration'>Користувач з таким Email та Категорією вже зареєстрований</p>";
      } else {
         ...
         if ($db_server->query($sqlInsert) === TRUE) {
            ...
            echo "<p class='error'>Помилка реєстрації, спробуйте ще раз</p>";
         }
      }
   }
}
