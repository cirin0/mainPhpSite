   ...
   if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if ($resultCheck > 0) {
         while ($row = mysqli_fetch_assoc($result)) {
            echo "<p class='success'>" . $row['login'] . "<br>" . "   Ваш '" . $row['user_category'] . "' пароль: " . $row['password'] . "</p>" . "<br>";
         }
      } else {
         echo "<p class='error_message'>Користувача з таким логіном та категорією не знайдено</p>";
      }
   }
   ?>
